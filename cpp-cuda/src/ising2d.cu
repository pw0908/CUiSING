 ...
 *    - const int d: dimension of system (2 or 3)
 * 
 * Outputs:
 *    - none (void)
 * 
...
__global__ void init_lattice(signed int *lattice,
    ...
    if (tid >= round(std::pow(n,d))) return;
    float r = rands[tid];
...
__global__ void cudaCalcHamiltonian2dKernel(signed int *lattice,
    ...
    if (threadIdx.x == 0)
        atomicAdd(&E[iter], shmem[0]/(n*n*(2.0*J+fabs(h))));
}
...
{
    CHECK_CURAND(curandGenerateUniform(cg,rands,round(std::pow(n,d))));
}
